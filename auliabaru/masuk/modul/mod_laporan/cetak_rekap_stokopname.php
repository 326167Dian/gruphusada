...
$pdf->Cell(10, 0.7, 'Nama Barang', 1, 0, 'L');
$pdf->Cell(1.5, 0.7, 'Satuan', 1, 0, 'C');
$pdf->Cell(1.5, 0.7, 'SS', 1, 0, 'C');
$pdf->Cell(1.5, 0.7, 'SF', 1, 0, 'C');
$pdf->Cell(2, 0.7, 'Exp Date', 1, 0, 'C');
$pdf->Cell(1.5, 0.7, 'Jml (ED)', 1, 0, 'C');
$pdf->Cell(1.5, 0.7, 'SF-SS', 1, 0, 'C');
...
$query = $db->query("SELECT * FROM stok_opname a 
            ...
            ORDER BY a.id_stok_opname DESC");

while ($r = $query->fetch_assoc()) {

    	$pdf->Cell(1, 0.6, $no , 1, 0, 'C');
    	$pdf->Cell(3.5, 0.6, $r['kd_barang'],1, 0, 'L');
    	$pdf->Cell(10, 0.6, $r['nm_barang'],1, 0, 'L');
    	$pdf->Cell(1.5, 0.6, $r['sat_barang'], 1, 0,'C');
    	$pdf->Cell(1.5, 0.6, $r['stok_sistem'], 1, 0,'C');
    	$pdf->Cell(1.5, 0.6, $r['stok_fisik'], 1, 0,'C');
    	$pdf->Cell(2, 0.6, $r['exp_date'], 1, 0,'C');
    	$pdf->Cell(1.5, 0.6, $r['jml'], 1, 0,'C');
    	$pdf->Cell(1.5, 0.6, $r['selisih'], 1, 0,'C');
...
$pdf->SetFont('Arial','',8);
$pdf->Cell(5.5, 0.5, "- ED = Expired Date",0,1,'L');
