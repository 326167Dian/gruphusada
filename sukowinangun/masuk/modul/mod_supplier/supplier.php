...
if (empty($_SESSION['username']) and empty($_SESSION['passuser'])) {
	...
	switch ($_GET['act']) {
		...
		default:
			...
			$tampil_supplier = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM supplier ORDER BY id_supplier DESC");
...
?>
							...
							while ($r = mysqli_fetch_array($tampil_supplier)) {
								...
								echo "<tr class='warnabaris' >
											<td>$no</td>           
											 ...
											 <td>$r[ket_supplier]</td>
											 <td>
											    <button type='button' class='btn btn-primary btn-xs' id='btn_data' data-id='$r[id_supplier]'>DATA OBAT</button>
											    <button type='button' class='btn btn-warning btn-xs' id='btn_edit' data-id='$r[id_supplier]'>EDIT</button>
		...
		case "edit":
		  ...
		  <div class='box box-primary box-solid'>
				...
				<div class='box-body table-responsive'>
						<form method=POST action=$aksi?module=supplier&act=update_supplier  enctype='multipart/form-data' class='form-horizontal' id='frmEditSupplier'>
	...
	<script>
            $(document).ready(function() {
                ...
                // table.page(startIndex).draw(false);
                ...
                $('#example11 tbody').on('click', '#btn_hapus', function () {
                    ...
                    if (confirm('Anda yakin ingin menghapus?') == true) {
                        $.ajax({
                    		url: 'modul/mod_supplier/aksi_supplier.php?module=supplier&act=hapus&id='+id,
                    		type: 'POST',
                    	}).success(function() {
                ...
                $('#example11 tbody').on('click', '#btn_edit', function () {
                    ...
                    location.href = '?module=supplier&act=edit&id='+id+'&page='+currentPage;
                });
                
                $('#example11 tbody').on('click', '#btn_data', function () {
                    var id = $(this).data('id');
                    var currentPage = table.page() + 1;
                    location.href = '?module=supplier&act=dataobat&id='+id+'&page='+currentPage;
                });
                        
                $('#btn_cancel').on('click', function(){
                    // var currentPage = $(this).data('page');
                    var currentPage = getPageFromUrl() + 1
                    location.href = '?module=supplier&page='+currentPage;
                ...
                $("#frmEditSupplier").submit(function(e) {
                    ...
                    $.ajax({
                        ...
                        success: function(data)
                        {
                            location.href = '?module=supplier&page='+vpage;
                        }
                    });
