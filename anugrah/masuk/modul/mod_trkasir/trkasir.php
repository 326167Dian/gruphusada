...
?>
                        ...
                        ";
                            ...
                            <table class="table table-striped table-bordered table-responsive">
                                ...
                                while ($tt=mysqli_fetch_array($tamtot)){
                                    ...
                                    $shift3= $db->query( "SELECT id_trkasir, kd_trkasir, SUM(ttl_trkasir) as ttlskrg1
                                        FROM trkasir WHERE tgl_trkasir='$tgl_awal' AND id_carabayar='$tt[id_carabayar]' and shift=3 ");
                                    $malam1 = $shift3->fetch_array();
                                    $malam = format_rupiah($malam1['ttlskrg1']);
                           ...
                           ";
                                ...
                                $totmalam = $db->query("SELECT id_trkasir, kd_trkasir, SUM(ttl_trkasir) as ttlskrg4                                                              
                                            FROM trkasir WHERE tgl_trkasir='$tgl_awal' and shift=3 ");
                                $tmalam = $totmalam->fetch_array();
                                $shiftmalam = format_rupiah($tmalam['ttlskrg4']);
			...
			<strong>Klik Tombol ini sebelum tutup transaksi setiap Shift</strong>
            <?php 
...
<?php
		...
		case "tambah":
			...
			if ($hitung < 1) {
				...
				// $cekkd = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM kdtk WHERE stt_kdtk='ON'");
				...
				if ($ketemucekkd > 0) {
					...
					if($ketemucekkd2 > 0){
					    $kdunik1 = date('dmyHis') + 1;
					    $kdtransaksi = "TKP-" . $kdunik1;
					} 
